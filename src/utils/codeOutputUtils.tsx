...
export function inferPropType(prop: string): { type: string; defaultValue: string } {
    const lower = prop.toLowerCase();

    if (prop.startsWith("on")) return { type: "() => void", defaultValue: "() => {}" };
    if (prop.startsWith("set")) return { type: "(value: string) => void", defaultValue: "() => {}" };
    if (["count", "age", "total", "number", "index"].some(k => lower.includes(k))) {
